...
| R4 | CSRF su mutazioni cookie | Alto / Bassa | backend | P0 | `csrf_origin_middleware` vs `ALLOWED_ORIGINS` (vale anche per il cookie remember), CORS allowlist, no wildcard+credenziali (`main.py`) | Fase 10: test CSRF/session/WS |
| R4b | Remember-token rubato riusato | Alto / Bassa | backend | P0 | token opaco, server solo sha256 (`remember_devices`), rotazione a ogni uso (replay muore al 2° tentativo), path cookie limitato ad auth, logout = fine trust, revoca per-device (`auth.py`, UI Trusted Devices) | Fase 8: audit accessi device + alert su rotazione fallita |
